...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (target >= minPos && target <= maxPos) {  // Validate target within calibrated range
      moveToPosition(target, 200);  // Move to user-defined position
...
void setMotor(int dir, int pwmVal, int pwm, int in1, int in2) {
  ...
  if (dir == 1) {  // Upward (IN1 LOW, IN2 HIGH)
    digitalWrite(in1, LOW);
    digitalWrite(in2, HIGH);
  } else if (dir == -1) {  // Downward (IN1 HIGH, IN2 LOW)
...
// Interrupt handler to update position based on IR sensor ( )
// void readSensor() {
//   static int lastState = HIGH;  // Track previous sensor state
//   int currentState = digitalRead(SENSOR_PIN);
//   if (currentState != lastState) {  // Detect edge change
//     if (digitalRead(IN1) == LOW && digitalRead(IN2) == HIGH) {  // Moving upward
//       if (digitalRead(UPPER_LIMIT_PIN) == HIGH) pos++;  // Increment if not at upper limit
//     } else if (digitalRead(IN1) == HIGH && digitalRead(IN2) == LOW) {  // Moving downward
//       if (digitalRead(LOWER_LIMIT_PIN) == HIGH) pos--;  // Decrement if not at lower limit
//     }
  ...
  if ((currentState != lastState) && (currentTime - lastInterruptTime >= debounceDelay)) {  // Detect edge change
    if (digitalRead(IN1) == LOW && digitalRead(IN2) == HIGH) {  // Moving upward
      if (digitalRead(UPPER_LIMIT_PIN) == HIGH) {
        ...
        // Serial.println(pos);
        }  // Increment if not at upper limit
    } else if (digitalRead(IN1) == HIGH && digitalRead(IN2) == LOW) {  // Moving downward
      if (digitalRead(LOWER_LIMIT_PIN) == HIGH)
      {
        ...
        // Serial.println(pos);
      }  // Decrement if not at lower limit
    }
...
void calibratePosition() {
  ...
  setMotor(-1, 200, PWM, IN1, IN2);  // Move downward (IN1 HIGH, IN2 LOW)
  ...
  setMotor(1, 200, PWM, IN1, IN2);  // Move upward (IN1 LOW, IN2 HIGH)
  ...
  moveToPosition(INITIAL_POS, 200);  // Move to initial position (15)
  pos = INITIAL_POS;
